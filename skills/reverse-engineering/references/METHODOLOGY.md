...
- **Наш вывод** — one of *использовать полностью / адаптировать / объединить / отложить / отвергнуть* — an analytical conclusion about what the evidence supports, not a build decision.
...
Priority against what else is in flight, and exactly where in the receiving system a finding lands, are not part of this card — those are implementation decisions for whoever owns the receiving system, not conclusions drawn from evidence. A card that names them anyway is out of scope, not more thorough.
...
**Provenance of this gate**: conditions 1, 2, 3, 6, 7 are each adapted from a specific cited practice above. Conditions 4 and 5 were tagged **original** — no reviewed discipline judges its object of study for the purpose of transplanting a capability into a separate, different system; intelligence analysis, SRE, and ADR practice all reason about and within their *own* system, not across systems. This was flagged as the highest-risk pair in this methodology, first candidate for revision if real cases show it producing false positives. Condition 8 is tagged **adapted** — the 2026 deep-research-agent literature explicitly names confident fabrication under insufficient evidence as the worst failure mode of the category (worse than an honest "cannot verify"), and names held-out abstention testing as the direct check against it — independently reconfirmed 2026-08-12 against `arxiv.org/html/2601.22984v1` ("Why Your Deep Research Agent Fails," DeepHalluBench), which names exactly this pattern in deployed systems (OpenAI, Grok, described as "confident fabricators," and names benchmark scoring that rewards guessing over abstention as the structural cause); this condition is that check applied to this methodology's own conclusions rather than to a benchmark question set.

**Correction (2026-08-12)**: the "original" tag on conditions 4 and 5 does not fully hold. Requested by Engineer (`ENGINEER/OPEN_QUESTIONS.md`, 2026-08-12 entry) as a self-audit of this whole methodology against an external reference set, not self-confirmation — cross-organizational best-practice benchmarking methodology, pioneered by Xerox (1979) and standardized industry-wide since (Rank Xerox–Fuji case studies, APQC), already performs exactly this move: identify a practice at a genuinely different organization, then explicitly decide what adapts to the receiving organization's own context and what does not — the discipline's own "practices learned from others must be adapted to an organization's culture, technology and human resources" principle, confirmed via `researchgate.net/publication/326113663` and the Rank Xerox retrospective literature. This was missed by the original P0 prior-art search and not caught until reverse-engineering was applied to this methodology as its own investigation target (full self-audit: `knowledge/CAPABILITY_MAP.md`, "External Validation" section, 2026-08-12). Re-tagged **adapted** (benchmarking practice) rather than original; the false-positive risk-flag on this pair is downgraded accordingly. The specific card structure in 5a (Назначение/Почему интересен/Наш вывод/Что конкретно переносим) remains this project's own elaboration — no source found specifies benchmarking transfer-decisions at that field-level granularity, so 5a itself keeps its original tag.
